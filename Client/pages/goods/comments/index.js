...
Page({
  ...
  onLoad(options) {
    console.log('[comments page] onLoad options:', options);
    const goodIdParam = options.goodId || options.good_id || '';
    const goodIdNum = parseInt(goodIdParam, 10) || 0;
    this.setData({ good_id: goodIdNum });
    if (goodIdNum) {
      this.loadAllComments(goodIdNum);
    }
  },
  onShow() {
    console.log('[comments page] onShow, good_id:', this.data.good_id);
    if (this.data.good_id) {
      ...
      const goodIdParam = query.goodId || query.good_id || '';
      const goodIdNum = parseInt(goodIdParam, 10) || 0;
      console.log('[comments page] onShow fallback query:', query, 'parsed:', goodIdNum);
      if (goodIdNum) {
        this.setData({ good_id: goodIdNum });
        this.loadAllComments(goodIdNum);
      }
  ...
  async loadAllComments(goodId) {
    try {
      wx.showLoading({ title: '加载评论', mask: false });
      const { getComment } = require('../../../services/comments/getComment');
      const res = await getComment({ good_id: goodId, land_id: 0 });
      console.log('[comments list] getComment response:', res);
      ...
      const pageList = list.map((c, idx) => {
        ...
        console.log(`[comments list] item#${idx}`, {
          id: c.id || c.ID,
      ...
      console.error('[comments list] load error:', e);
  ...
  async onToggleReplies(e) {
    ...
    console.log('[toggle replies] click:', { id, index });
    ...
    try {
      ...
      console.log('[toggle replies] getCommentReply response:', res);
      ...
      console.error('[toggle replies] error:', err);
