...
export async function setAddRoute() {
	...
	console.log('要添加的路由数量:', routes.length);
	await routes.forEach((route: RouteRecordRaw) => {
		router.addRoute(route);
		console.log('添加路由:', route.name, route.path);
	});
	console.log('路由添加完成，当前路由总数:', router.getRoutes().length);
}
...
export function setCacheTagsViewRoutes() {
	...
	let rolesRoutes = setFilterHasRolesMenu(dynamicRoutes, ['admin', 'common']);
...
export function setFilterMenuAndCacheTagsViewRoutes() {
	...
	const routes = setFilterHasRolesMenu(dynamicRoutes[0].children, ['admin', 'common']);
	console.log('设置的路由列表:', routes);
