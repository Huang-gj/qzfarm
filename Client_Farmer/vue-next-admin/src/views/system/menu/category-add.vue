...
<script setup lang="ts" name="productCategoryAdd">
...
const onSubmit = async () => {
	...
	try {
		...
		if (responseCode === 200) {
			...
			ElMessage.success('农产品类目新增成功！');
			resetForm();
			// 重新获取类目列表
			await fetchCategoryList();
		} else {
			const errorMsg = responseMsg || '新增农产品类目失败';
			ElMessage.error(errorMsg);
		}
	} catch (error: any) {
		console.error('新增农产品类目失败:', error);
		ElMessage.error(error.message || '新增农产品类目失败，请稍后重试');
	} finally {
