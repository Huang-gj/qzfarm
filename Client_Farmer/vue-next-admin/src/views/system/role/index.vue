...
const fetchLandOrders = async () => {
	try {
		...
		const params: GetLandOrderRequest = { farm_id: Number(farmId) };
		const response: any = await getLandOrder(params);
...
const handleRefresh = () => { searchForm.orderId = ''; searchForm.userId = ''; searchForm.orderStatus = ''; pagination.currentPage = 1; fetchLandOrders(); };
const handleViewDetail = (row: LandOrder) => { currentOrder.value = row; dialogVisible.value = true; };
...
onMounted(() => { fetchLandOrders(); });
</script>

<style scoped lang="scss">
.system-land-order-container {
	.system-land-order-search { display: flex; align-items: center; flex-wrap: wrap; gap: 10px; }
	.system-land-order-pagination { display: flex; justify-content: flex-end; }
