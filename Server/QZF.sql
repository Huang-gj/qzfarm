...
CREATE TABLE farms
(
    farm_id            VARCHAR(128) PRIMARY KEY,            -- 农场ID
    farm_name          VARCHAR(128) NOT NULL,               -- 农场名称
    owner_id           VARCHAR(128),                        -- 农场主用户ID
...
(
    user_id            VARCHAR(128) PRIMARY KEY,               -- 用户ID
    ...
    farming_experience INT     DEFAULT 0,                      -- 农场经营经验年数
    ...
    preferred_farm_id  VARCHAR(128),                           -- 用户偏好的农场ID
...
(
    address_id        VARCHAR(128) PRIMARY KEY,      -- 地址ID
...
(
    category_id   VARCHAR(128) PRIMARY KEY,                             -- 分类ID
    farm_id       VARCHAR(128),                                         -- 关联农场ID（若为特定农场的分类）
    name          VARCHAR(128) NOT NULL,                                -- 分类名称
    parent_id     VARCHAR(128),                                         -- 父分类ID（若有）
    description   TEXT,                                                 -- 分类描述
    image_url     VARCHAR(512),                                         -- 分类图片
    display_order INT       DEFAULT 0,                                  -- 显示顺序
    created_at    TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    FOREIGN KEY (parent_id) REFERENCES product_categories (category_id), -- 自引用外键
...
(
    product_id        VARCHAR(128) PRIMARY KEY,                           -- 商品ID
    farm_id           VARCHAR(128)   NOT NULL,                            -- 关联农场ID
    ...
    category_id       VARCHAR(128),                                       -- 商品分类ID
    ...
    harvest_id        VARCHAR(128),                                       -- 关联的收获记录ID
    farm_unit_id      VARCHAR(128),                                       -- 关联的认养单元ID
...
(
    summary_id      VARCHAR(128) PRIMARY KEY,                      -- 评论汇总记录ID
    product_id      VARCHAR(128),                                  -- 商品ID
    order_no        VARCHAR(128),                                  -- 订单号
    logistics_score INT,                                           -- 物流评分
    service_score   INT,                                           -- 服务评分
    created_at      TIMESTAMP DEFAULT CURRENT_TIMESTAMP,           -- 创建时间
    farm_id         VARCHAR(128),                                  -- 关联农场ID
...
(
    id                        VARCHAR(128) PRIMARY KEY,                       -- 评论ID
    summary_id                VARCHAR(128),                                   -- 关联商品评论汇总表ID
    uid                       VARCHAR(128),                                   -- 用户ID
    ...
    farm_id                   VARCHAR(128),                                   -- 关联农场ID
    farm_unit_id              VARCHAR(128),                                   -- 认养单元ID
    adoption_experience_score INT,                                            -- 认养体验评分
    farm_service_score        INT,                                            -- 农场服务评分
...
(
    media_id   SERIAL PRIMARY KEY,                            -- 媒体资源ID
    comment_id VARCHAR(128),                                  -- 关联评论ID
...
(
    reply_id      SERIAL PRIMARY KEY,                          -- 回复记录ID
    comment_id    VARCHAR(128),                                -- 关联评论ID
    reply_content TEXT,                                        -- 回复内容
    reply_user_id VARCHAR(128),                                -- 回复用户ID（商家或用户）
...
(
    settle_id              VARCHAR(128) PRIMARY KEY,                      -- 结算ID
    ...
    user_address_id        VARCHAR(128),                                  -- 用户地址ID
    created_at             TIMESTAMP DEFAULT CURRENT_TIMESTAMP,           -- 创建时间
    order_type             INT       DEFAULT 1,                           -- 订单类型
    adoption_id            VARCHAR(128),                                  -- 认养ID
    farm_id                VARCHAR(128),                                  -- 关联农场ID
    farm_unit_id           VARCHAR(128),                                  -- 认养单元ID
...
(
    detail_id         VARCHAR(128) PRIMARY KEY,                          -- 订单商品详情ID
    settle_id         VARCHAR(128) NOT NULL,                             -- 关联结算ID
    product_id        VARCHAR(128) NOT NULL,                             -- 商品ID
    farm_id           VARCHAR(128) NOT NULL,                             -- 关联农场ID
...
(
    coupon_id                VARCHAR(128) PRIMARY KEY,                   -- 优惠券ID
    settle_id                VARCHAR(128),                               -- 结算ID
...
(
    delivery_id               VARCHAR(128) PRIMARY KEY,                  -- 配送ID
    settle_id                 VARCHAR(128),                              -- 关联结算ID
    ...
    farm_pickup_available     BOOLEAN   DEFAULT FALSE,                   -- 是否支持农场自取
    FOREIGN KEY (settle_id) REFERENCES order_settle_summary (settle_id), -- 外键，关联订单结算表
    FOREIGN KEY (farm_id) REFERENCES farms (farm_id)                     -- 外键，关联农场表
)ENGINE = InnoDB
CHARACTER SET = utf8mb4
...
(
    promotion_id       VARCHAR(128) PRIMARY KEY,     -- 促销活动ID
...
(
    ...
    farm_id     VARCHAR(128),                             -- 关联农场ID（若搜索特定农场）
    FOREIGN KEY (user_id) REFERENCES user_info (user_id), -- 外键，关联用户信息表
    FOREIGN KEY (farm_id) REFERENCES farms (farm_id)      -- 外键，关联农场表
)ENGINE = InnoDB
CHARACTER SET = utf8mb4
...
(
    ...
    /* 农场积分扩展 */
    farm_id       VARCHAR(128),                           -- 关联农场ID（特定农场的积分）
...
(
    area_id        VARCHAR(128) PRIMARY KEY,            -- 区域ID
    farm_id        VARCHAR(128) NOT NULL,               -- 关联农场ID
...
(
    unit_id             VARCHAR(128) PRIMARY KEY,            -- 单元ID
    farm_id             VARCHAR(128) NOT NULL,               -- 关联农场ID
    area_id             VARCHAR(128),                        -- 关联区域ID
...
(
    adoption_id          VARCHAR(128) PRIMARY KEY,            -- 认养订单ID
    user_id              VARCHAR(128),                        -- 用户ID
    farm_id              VARCHAR(128) NOT NULL,               -- 关联农场ID
    unit_id              VARCHAR(128),                        -- 关联农场单元ID
...
(
    harvest_id      VARCHAR(128) PRIMARY KEY,                           -- 收获记录ID
    farm_id         VARCHAR(128) NOT NULL,                              -- 关联农场ID
    adoption_id     VARCHAR(128),                                       -- 关联认养订单ID
    user_id         VARCHAR(128),                                       -- 用户ID
    unit_id         VARCHAR(128),                                       -- 关联农场单元ID
...
(
    activity_id   VARCHAR(128) PRIMARY KEY,                            -- 活动ID
    farm_id       VARCHAR(128) NOT NULL,                               -- 关联农场ID
    unit_id       VARCHAR(128),                                        -- 关联农场单元ID
    adoption_id   VARCHAR(128),                                        -- 关联认养订单ID
...
(
    plan_id         VARCHAR(128) PRIMARY KEY,            -- 周期计划ID
    farm_id         VARCHAR(128) NOT NULL,               -- 关联农场ID
    unit_type       INT,                                 -- 单元类型
...
(
    monitor_id VARCHAR(128) PRIMARY KEY,                  -- 监控ID
    farm_id    VARCHAR(128) NOT NULL,                     -- 关联农场ID
    unit_id    VARCHAR(128),                              -- 关联农场单元ID
...
(
    review_id  VARCHAR(128) PRIMARY KEY,                 -- 评价ID
    farm_id    VARCHAR(128) NOT NULL,                    -- 关联农场ID
    user_id    VARCHAR(128) NOT NULL,                    -- 关联用户ID
    content    TEXT,                                     -- 评价内容
...
/** 农场管理员表 (farm_managers) **/
DROP TABLE IF EXISTS `farm_managers`;
CREATE TABLE farm_managers
(
    manager_id  VARCHAR(128) PRIMARY KEY,                -- 管理员ID
    farm_id     VARCHAR(128) NOT NULL,                   -- 关联农场ID
    user_id     VARCHAR(128) NOT NULL,                   -- 关联用户ID
...
(
    booking_id VARCHAR(128) PRIMARY KEY,                 -- 预约ID
    farm_id    VARCHAR(128) NOT NULL,                    -- 关联农场ID
...
(
    announcement_id VARCHAR(128) PRIMARY KEY,            -- 公告ID
    farm_id         VARCHAR(128) NOT NULL,               -- 关联农场ID
...
(
    equipment_id   VARCHAR(128) PRIMARY KEY,            -- 设备ID
    farm_id        VARCHAR(128) NOT NULL,               -- 关联农场ID
...
/** 库存变动记录表 (inventory_changes) **/
DROP TABLE IF EXISTS `inventory_changes`;
CREATE TABLE inventory_changes
(
    change_id   VARCHAR(128) PRIMARY KEY,                           -- 变动记录ID
    product_id  VARCHAR(128) NOT NULL,                              -- 商品ID
    farm_id     VARCHAR(128) NOT NULL,                              -- 关联农场ID
    change_type INT,                                                -- 变动类型(1=入库,2=出库,3=损耗)
    quantity    INT          NOT NULL,                              -- 变动数量
    reason      VARCHAR(128),                                       -- 变动原因
    operator_id VARCHAR(128),                                       -- 操作人员ID
    change_time TIMESTAMP DEFAULT CURRENT_TIMESTAMP,                -- 变动时间
    FOREIGN KEY (product_id) REFERENCES farm_products (product_id), -- 外键，关联商品表
    FOREIGN KEY (farm_id) REFERENCES farms (farm_id),               -- 外键，关联农场表
    FOREIGN KEY (operator_id) REFERENCES user_info (user_id)        -- 外键，关联用户信息表
)ENGINE = InnoDB
CHARACTER SET = utf8mb4
COLLATE = utf8mb4_general_ci
ROW_FORMAT = Dynamic;
...
(
    article_id   VARCHAR(128) PRIMARY KEY,                 -- 文章ID
    farm_id      VARCHAR(128) NOT NULL,                    -- 关联农场ID
