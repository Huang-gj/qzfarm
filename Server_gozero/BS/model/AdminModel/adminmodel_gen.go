...
func (m *defaultAdminModel) Update(ctx context.Context, data *Admin) error {
	query := fmt.Sprintf("update %s set `avatar` = ?, `nickname` = ?, `qq_email` = ?, `gender` = ? where `admin_id` = ?", m.table)
	_, err := m.conn.ExecCtx(ctx, query, data.Avatar, data.Nickname, data.QqEmail, data.Gender, data.AdminId)
	return err
}
