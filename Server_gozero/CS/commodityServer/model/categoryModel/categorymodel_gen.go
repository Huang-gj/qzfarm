...
type (
	...
	Category struct {
		...
		Text       sql.NullString `db:"text"`        // 分类备注
		ImageUrl   string         `db:"image_url"`   // 种类标识图标
	}
)
...
func (m *defaultCategoryModel) Insert(ctx context.Context, data *Category) (sql.Result, error) {
	...
	ret, err := m.conn.ExecCtx(ctx, query, data.DelState, data.DelTime, data.CategoryId, data.Name, data.CateType, data.Text, data.ImageUrl)
	return ret, err
}
...
func (m *defaultCategoryModel) Update(ctx context.Context, data *Category) error {
	...
	_, err := m.conn.ExecCtx(ctx, query, data.DelState, data.DelTime, data.CategoryId, data.Name, data.CateType, data.Text, data.ImageUrl, data.Id)
	return err
}
